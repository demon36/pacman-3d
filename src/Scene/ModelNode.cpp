...
void ModelNode::Render(ShaderProgram* shaderProgram, glm::mat4 parentTransforms)
{
	...
	GLuint vertexPosition_modelspaceID = glGetAttribLocation(shaderProgram->programID, "vertexPosition_modelspace");
	GLuint inputColorID = glGetAttribLocation(shaderProgram->programID, "inputColor");
	GLuint texCoordID = glGetAttribLocation(shaderProgram->programID, "texCoord");
	GLuint vertexNormal_modelspaceID = glGetAttribLocation(shaderProgram->programID,"vertexNormal_modelspace");
	for (int i = 0; i < meshes.size(); i++)
	{
		meshes[i]->Draw(vertexPosition_modelspaceID,inputColorID,texCoordID,vertexNormal_modelspaceID);
	}
