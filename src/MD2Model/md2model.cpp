...
void CMD2Model::RenderModel(animState_t* animState, KeyFrameAnimationShader* shader)
{
	...
	GLuint vertexPosition_modelspaceID = glGetAttribLocation(shader->programID,"vertexPosition_modelspace");
	GLuint texCoordID = glGetAttribLocation(shader->programID,"texCoord");
	GLuint vertexNormal_modelspaceID = glGetAttribLocation(shader->programID,"vertexNormal_modelspace");
	GLuint nextPosition_modelspaceID = glGetAttribLocation(shader->programID,"nextPosition_modelspace");
	GLuint nextNormal_modelspaceID = glGetAttribLocation(shader->programID,"nextNormal_modelspace");
	...
	if(animState == NULL)
	{
		...
		glVertexAttribPointer(vertexPosition_modelspaceID, 3, GL_FLOAT, GL_FALSE, 2*sizeof(glm::vec3), 0);
	...
	else
	{
		...
		glEnableVertexAttribArray(nextPosition_modelspaceID);
		vboFrameVertices[animState->next_frame].BindVBO();
		glVertexAttribPointer(nextPosition_modelspaceID, 3, GL_FLOAT, GL_FALSE, 2*sizeof(glm::vec3), 0);

		// Change normal pointers to current frame

		glEnableVertexAttribArray(vertexNormal_modelspaceID);
		vboFrameVertices[animState->curr_frame].BindVBO();
		glVertexAttribPointer(vertexNormal_modelspaceID, 3, GL_FLOAT, GL_FALSE, 2*sizeof(glm::vec3), (void*)sizeof(glm::vec3));

		glEnableVertexAttribArray(nextNormal_modelspaceID);
		vboFrameVertices[animState->next_frame].BindVBO();
		glVertexAttribPointer(nextNormal_modelspaceID, 3, GL_FLOAT, GL_FALSE, 2*sizeof(glm::vec3), (void*)sizeof(glm::vec3));
