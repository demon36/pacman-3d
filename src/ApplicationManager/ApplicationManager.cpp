...
int ApplicationManager::MouseScrolled = 0;
...
bool ApplicationManager::InitalizeApplication(int pWindowSizeWidth, int pWindowSizeHeight)
{
	...
	mWindow = glfwCreateWindow(pWindowSizeWidth, pWindowSizeWidth, "PACMAN ", glfwGetPrimaryMonitor(), NULL);//NULL, NULL);
	if( mWindow == nullptr ){
	...
	glfwSetScrollCallback(mWindow, &this->MouseScrolled_);
...
void ApplicationManager::StartMainLoop()
{
	...
	do 
	{
		...
		{
			if(mRenderer->cameraType == 0)
				mRenderer->HandleKeyboardInput(KeyPressed);
			else if(mRenderer->cameraType == 1)
				mRenderer->HandleKeyboardInput_tpc(KeyPressed);
			//reset the pressed key.
			KeyPressed = -1;
		}

		//check if mouse wheel is scrolled
		if(ApplicationManager::MouseScrolled != 0){
			mRenderer->HandleScroll_tpc(MouseScrolled);
			MouseScrolled = 0;
		}
		 ...
		 || ApplicationManager::MouseYPos != ApplicationManager::WindowSizeHeight/2)
		{
			...
			if(mRenderer->cameraType == 0)
				...
				mRenderer->HandleMouse_tpc( movedDistanceX, movedDistanceY);
		...
		//sleep to limit framerate trying to normalize game speed
		//Sleep(2);
...
// mouse scroll event
void ApplicationManager::MouseScrolled_(GLFWwindow* window, double xoffset, double yoffset)
{
	MouseScrolled = yoffset;
}
